                ...
                <div class="card card-flush h-100">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="row g-3" id="discount_type_options">
                                @php
                                    $types = [
                                        ...
                                        'special_price' => [
                                            ...
                                            'desc' =>
                                                app()->getLocale() === 'ar'
                                                    ? 'مثال: سعر أول حجز = 49 ريال'
                                                    : 'E.g. First booking price = 49 SAR',
                                            'icon' => 'ki-star',
                                            'color' => 'warning',
                                        ],
                                    ];
                                ...
                                @foreach ($types as $typeKey => $typeDef)
                                    <div class="col-md-4">
                                        <label
                                            class="discount-type-card d-flex flex-column align-items-center p-5 border border-2 rounded cursor-pointer
                                        {{ old('discount_type', $config['discount_type']) === $typeKey ? 'border-' . $typeDef['color'] . ' bg-light-' . $typeDef['color'] : 'border-dashed border-gray-300' }}"
                                            ...
                                            <i
                                                class="ki-duotone {{ $typeDef['icon'] }} fs-2x text-{{ $typeDef['color'] }} mb-2">
                                                <span class="path1"></span><span class="path2"></span>
                                            </i>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="input-group">
                                <input type="number" name="discount_value" id="discount_value"
                                    ...
                                    value="{{ old('discount_value', $config['discount_value']) }}" min="0"
                                    step="0.01">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="applies_to_service_ids[]" class="form-select" data-control="select2"
                    ...
                    <div class="card-body pt-0">
                        <div class="bg-light-primary rounded p-4">
                            <div class="d-flex justify-content-between mb-2">
                                <span
                                    class="text-muted fs-7">{{ app()->getLocale() === 'ar' ? 'السعر الأصلي' : 'Original Price' }}</span>
                                <span class="fw-bold" id="preview_original">100 SAR</span>
                            </div>
                            <div class="d-flex justify-content-between mb-2 text-danger">
                                <span
                                    class="fs-7">{{ app()->getLocale() === 'ar' ? 'خصم أول حجز' : 'First Booking Discount' }}</span>
                                <span class="fw-bold" id="preview_discount">-0 SAR</span>
                            </div>
                            <div class="separator separator-dashed my-2"></div>
                            <div class="d-flex justify-content-between">
                                <span
                                    class="fw-bold">{{ app()->getLocale() === 'ar' ? 'السعر النهائي' : 'Final Price' }}</span>
...
@push('custom-script')
    <script>
        (function() {
            ...
            const hints = {
                percentage: isAr ? 'أدخل نسبة من 0 إلى 100 (مثال: 20 = خصم 20%)' :
                    'Enter 0–100 (e.g. 20 = 20% off)',
                fixed: isAr ? 'أدخل مبلغ الخصم بالريال (مثال: 30 = خصم 30 ريال)' : 'Discount amount in SAR',
                special_price: isAr ?
                    '⚠️ أدخل السعر النهائي لأول حجز — يجب أن يكون أقل من سعر الخدمة الأصلي' :
                    '⚠️ Enter the final price — must be less than the original service price',
            };
