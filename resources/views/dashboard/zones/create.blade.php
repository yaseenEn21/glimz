...
<div class="card">
    <form id="zone_form" action="{{ route('dashboard.zones.store') }}" method="POST">
        @csrf
                    ...
                    <div class="card mb-7">
                        <div class="card-header border-0 pt-5">
                            <h3 class="card-title flex-column">
                                <span class="card-label fw-bold fs-3 mb-1">{{ __('zones.basic_data') }}</span>
                        ...
                        <div class="card-body pt-0">
                            <div class="row g-6">
                                ...
                                <div class="col-12 fv-row">
                                    ...
                                    {{-- نخزن الإحداثيات هنا (JSON) --}}
                                    <input type="hidden" name="polygon" id="zone_polygon" value="{{ old('polygon') }}">

                                    <div id="zone_map" class="mb-3"></div>

                                    {{-- خطأ الحقل (خصوصًا لأنه hidden) --}}
                                    <div class="invalid-feedback d-block" id="polygon_error"></div>

                                    <div class="form-text">
                                        {{ app()->getLocale() === 'ar'
                                            ? 'ارسم مضلع واحد لتحديد المنطقة باستخدام أداة الرسم أعلى الخريطة، ويمكنك تعديل النقاط بالسحب.'
                                            : 'Draw a single polygon using the toolbar. You can drag points to adjust it.' }}
                                    </div>
                                </div>
...
(function () {
        ...
        $.ajax({
            ...
            success: function(res) {
                Swal.fire({
                    icon: 'success',
                    title: "{{ __('zones.done') }}",
                    text: res.message || "{{ __('zones.created_successfully') }}",
                    timer: 2000,
            ...
            error: function(xhr) {
                if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                    ...
                    // لأن polygon hidden: اعرض الخطأ في div واضح
    ...
    window.initZoneMap = function() {
        ...
        // غيّرها حسب دولتك/مدينتك لو تحب
