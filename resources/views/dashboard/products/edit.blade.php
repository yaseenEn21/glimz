...
@php
    ...
    $nameAr = old('name.ar', $product->name['ar'] ?? '');
    $nameEn = old('name.en', $product->name['en'] ?? '');
    $descAr = old('description.ar', $product->description['ar'] ?? '');
    $descEn = old('description.en', $product->description['en'] ?? '');
    $price = old('price', $product->price);
    $discountPrice = old('discounted_price', $product->discounted_price);
    $maxQty = old('max_qty_per_booking', $product->max_qty_per_booking);
    $catId = old('product_category_id', $product->product_category_id);
    $sortOrder = old('sort_order', $product->sort_order ?? '');
    $isActiveOld = old('is_active', $product->is_active ? 1 : 0);
    $isActive = (bool) $isActiveOld;
@endphp

<form id="product_edit_form" action="{{ route('dashboard.products.update', $product->id) }}" method="POST"
                            ...
                            <div class="border border-dashed border-gray-300 rounded p-6 h-100">
                                <div class="mb-5">
                                    ...
                                    <input type="text" name="name[ar]" class="form-control form-control-lg bg-light"
                                        value="{{ $nameAr }}" placeholder="مثال: معطر سيارات" />
                                    <div class="invalid-feedback"></div>
                                </div>
                                <div>
                                    <label
                                        class="fs-6 fw-bold mb-2 text-gray-700">{{ __('products.description_ar') }}</label>
                                    <textarea name="description[ar]" class="form-control bg-light" rows="4" placeholder="وصف مختصر للمنتج">{{ $descAr }}</textarea>
                                    <div class="invalid-feedback"></div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="border border-dashed border-gray-300 rounded p-6 h-100">
                                <div class="mb-5">
                                    ...
                                    <input type="text" name="name[en]" class="form-control form-control-lg bg-light"
                                        value="{{ $nameEn }}" placeholder="Example: Car Freshener" />
                                    <div class="invalid-feedback"></div>
                                </div>
                                <div>
                                    <label
                                        class="fs-6 fw-bold mb-2 text-gray-700">{{ __('products.description_en') }}</label>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="position-relative">
                                <input type="number" min="1" name="max_qty_per_booking"
                                    class="form-control form-control-lg" value="{{ $maxQty }}"
                    ...
                    <div class="mb-10">
                        <div class="bg-light-primary rounded p-5">
                            ...
                            <div class="dropzone-custom border-2 border-dashed border-primary rounded text-center p-8"
                                ...
                                <div class="preview-container {{ $product->hasMedia('image_ar') ? '' : 'd-none' }}">
                                    <img src="{{ $imgAr }}" class="mw-100 rounded" style="max-height: 180px;"
                                        id="image_ar_preview" />
                                    <div class="mt-4">
                                ...
                                <div class="upload-prompt {{ $product->hasMedia('image_ar') ? 'd-none' : '' }}">
                    ...
                    <div>
                        <div class="bg-light-success rounded p-5">
                            ...
                            <div class="dropzone-custom border-2 border-dashed border-success rounded text-center p-8"
                                ...
                                <div class="preview-container {{ $product->hasMedia('image_en') ? '' : 'd-none' }}">
                                    <img src="{{ $imgEn }}" class="mw-100 rounded" style="max-height: 180px;"
                                        id="image_en_preview" />
                                    <div class="mt-4">
                                ...
                                <div class="upload-prompt {{ $product->hasMedia('image_en') ? 'd-none' : '' }}">
    ...
    (function() {
        ...
        $form.on('submit', function(e) {
            ...
            window.KH.setFormLoading($form, true, {
                text: '{{ app()->getLocale() === 'ar' ? 'جاري التحديث...' : 'Updating...' }}'
            });
            ...
            $.ajax({
                ...
                success: function(res) {
                    Swal.fire({
                        ...
                        title: 'تم',
                        text: res.message ||
                            '{{ __('products.updated_successfully') }}',
                        timer: 2000,
                ...
                error: function(xhr) {
                    if (xhr.status === 422 && xhr.responseJSON && xhr.responseJSON.errors) {
                        ...
                        let msg =
                            '{{ app()->getLocale() === 'ar' ? 'حدث خطأ غير متوقع.' : 'Unexpected error.' }}';
