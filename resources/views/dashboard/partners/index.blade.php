        ...
        <div class="d-flex flex-wrap align-items-center gap-4">
            ...
            <div class="flex-grow-1" style="min-width: 250px;">
                <div class="position-relative">
                    ...
                    <input type="text" id="search_name" class="form-control form-control-solid input-with-icon"
                        placeholder="{{ __('partners.search_by_name_placeholder') }}" />
                </div>
            </div>
            ...
            <div style="min-width: 180px;">
                <select id="filter_status" class="form-select form-select-solid" data-control="select2"
                    data-placeholder="{{ __('partners.all_statuses') }}" data-allow-clear="true">
                    <option value="">{{ __('partners.all_statuses') }}</option>
                    <option value="active">{{ __('partners.active') }}</option>
                    <option value="inactive">{{ __('partners.inactive') }}</option>
                </select>
            </div>
...
<div class="card shadow-sm">
    <div class="card-body">
        <div class="table-responsive">
            <table id="kt_partners_table" class="table table-row-bordered table-hover gy-5">
                <thead>
...
<script>
    (function() {
        ...
        const table = window.KH.initAjaxDatatable({
            tableId: 'kt_partners_table',
            ajaxUrl: '{{ route('dashboard.partners.index') }}',
            ...
            columns: [{
                ...
                },
                {
                    data: 'name',
                    name: 'name',
                    title: "{{ __('partners.fields.name') }}"
                },
                {
                    data: 'username',
                    name: 'username',
                    title: "{{ __('partners.fields.username') }}"
                },
                {
                    data: 'services_count',
                    name: 'services_count',
                    title: "{{ __('partners.fields.services_count') }}",
                    orderable: false,
                    searchable: false
                },
                {
                    ...
                    name: 'is_active',
                    title: "{{ __('partners.fields.status') }}",
                    orderable: true,
                    searchable: false
                },
                {
        ...
        $(document).on('click', '.js-delete-partner', function(e) {
            e.preventDefault();

            const $btn = $(this);
            const partnerId = $btn.data('id');
            const deleteUrl = '{{ route('dashboard.partners.destroy', ':id') }}'.replace(':id', partnerId);
            ...
            $('#confirmDeleteBtn').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // ✅ عرض Toast للنجاح
                        showToast('success',
                            '{{ __('partners.deleted_successfully') }}',
